...
export class AccountService {
  ...
  purchase(stock: Stock): void {
    ...
    if (stock.price < this.balance) {
      ...
      this.calculateValue();
      this.cacheValue();
      this.alertService.alert(`You bought ${stock.symbol}` + this.currencyPipe.transform(stock.price, 'USD', true, '.2'), 'success');
    } else {
      this.alertService.alert(`You have insufficient founds to buy ${stock.symbol}`, 'danger');
    }
  }
